...
The ShuffleNet student, at 42.74K parameters (41.6$\times$ compression), exceeds the teacher on 8 of the 11 evaluated scenarios, with the most pronounced gains on LOS B (93.1\% vs.\ 82.4\%, $+$10.7\%) and Walk B (94.8\% vs.\ 86.4\%, $+$8.4\%).
The DenseNet student achieves the most dramatic compression---from 7,472.38K to 8.96K parameters (834$\times$) and from 366.07M to 1.60M FLOPs (228.8$\times$)---while outperforming the teacher on 11 of the 11 scenarios, including NLOS D (92.2\% vs.\ 90.2\%) and Mobile Meeting (98.5\% vs.\ 93.2\%).
...
On the Jetson Nano, the ResNet18 teacher backbone requires 64.84~ms for feature extraction alone, yielding only 15~FPS and making it unsuitable for real-time per-packet authentication.
In contrast, the SCSKNet and ShuffleNet student backbones complete feature extraction in 4.75~ms, achieving 167~FPS on the same constrained hardware---a throughput that exceeds typical LoRa packet rates with substantial headroom.
With PCA-reduced embedding dimensions accelerating the KNN classification stage, end-to-end latency drops to 6.00~ms.

On the RTX~4090, the student backbones execute within 0.24--0.25~ms per inference.
With PCA-accelerated classification, end-to-end throughput reaches 1,064~FPS for the SCSKNet and ShuffleNet students and 1,316~FPS for the DenseNet student.
