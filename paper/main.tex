...
Experiments across five diverse architectures on a real-world LoRa dataset demonstrate 14--206$\times$ parameter compression with minimal accuracy degradation, and show that self-distillation can even purify representations when the teacher is undertrained, with the student consistently outperforming its teacher.
\end{abstract}
...
On LOS channels A and C, the student achieves 95.0\% and 95.5\% respectively, closely tracking the teacher's 99.7\% and 99.8\%.
Notably, the student surpasses the teacher on the NLOS E channel (81.1\% vs.\ 62.5\%) and on Walk B (93.5\% vs.\ 92.6\%), suggesting that the PCA-derived compact architecture is less prone to overfitting under channel degradation.
...
The ShuffleNet student, at 42.74K parameters (41.6$\times$ compression), exceeds the teacher on 9 of the 11 evaluated scenarios, with the most pronounced gains on LOS B (93.1\% vs.\ 82.4\%, $+$10.7\%) and Walk B (94.8\% vs.\ 86.4\%, $+$8.4\%).
The DenseNet student achieves the most dramatic compression---from 7,472.38K to 8.96K parameters (834$\times$) and from 366.07M to 1.60M FLOPs (228.8$\times$)---while outperforming the teacher on 10 of the 11 scenarios, including NLOS D (92.2\% vs.\ 90.2\%) and Mobile Meeting (98.5\% vs.\ 93.2\%).
...
To evaluate real-time capability, we measure inference latency on the NVIDIA Jetson Nano (4~GB).
The PCA-derived ResNet student completes feature extraction in \textbf{4.8~ms} per packet, compared to 64.8~ms for the original ResNet teacher---a 13.5$\times$ speedup.
With PCA-based classification backend acceleration, end-to-end latency reaches 5.8~ms, corresponding to \textbf{173~FPS}.
This throughput exceeds the requirements of typical LoRa packet rates by a wide margin, confirming that the automated pipeline produces not only parameter-efficient but also latency-efficient models suitable for real-time edge deployment.
...
Experiments across five diverse architectures on a real-world LoRa dataset demonstrate 14--206$\times$ parameter compression with minimal accuracy degradation, and edge deployment on an NVIDIA Jetson Nano achieves 173~FPS inference throughput.
